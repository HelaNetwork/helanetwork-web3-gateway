...
func TestEth_FeeHistory(t *testing.T) {
	...
	feeHistory, err := ec.FeeHistory(context.Background(), 4, big.NewInt(0), []float64{25, 75})
	require.Nil(t, err, "get FeeHistory")

	oldestBlock := feeHistory.OldestBlock
	reward := feeHistory.Reward
	baseFee := feeHistory.BaseFee
	gasUsedRatio := feeHistory.GasUsedRatio

	// Now you can use the individual fields as needed
	fmt.Printf("Oldest Block: %v\n", oldestBlock)
	fmt.Printf("Reward: %v\n", reward)
	fmt.Printf("Base Fee: %v\n", baseFee)
	fmt.Printf("Gas Used Ratio: %v\n", gasUsedRatio)
	
	t.Logf("oldestBlock: %v", oldestBlock)
}
