...
// This method was introduced with EIP 1559.

// FeeHistory returns data relevant for fee estimation based on the specified range of blocks (blockCount).
...
// or blocks older than a certain age (specified in maxHistory 1024 by default).
// Three arrays are returned based on the processed blocks:
//   - reward: the requested percentiles of effective priority fees per gas of transactions in each
//     block, sorted in ascending order and weighted by gas used. (0 in HELA ecosystem)
//   - baseFee: base fee per gas in the given block
//   - gasUsedRatio: gasUsed/gasLimit in the given block
//
// Note: baseFee includes the next block after the newest of the returned range, because this
// value can be derived from the newest block.
func (m *metricsWrapper) FeeHistory(ctx context.Context, blockCount hexutil.Uint, lastBlock ethrpc.BlockNumber, rewardPercentiles []float64) (res map[string]interface{}, err error) {
...
// ManagestShowProposal implements eth.API.
func (m *metricsWrapper) ManagestShowProposal(ctx context.Context, proposalID hexutil.Uint) (res map[string]interface{}, err error) {
	r, s, f, i, d := metrics.GetAPIMethodMetrics("eth_managestShowProposal")
	defer metrics.InstrumentCaller(r, s, f, i, d, &err)()

	res, err = m.api.ManagestShowProposal(ctx, proposalID)
	return
}

// ManagestShowRoles implements eth.API.
func (m *metricsWrapper) ManagestShowRoles(ctx context.Context, roleStr string) (res map[string]interface{}, err error) {
	r, s, f, i, d := metrics.GetAPIMethodMetrics("eth_managestShowRoles")
	defer metrics.InstrumentCaller(r, s, f, i, d, &err)()

	res, err = m.api.ManagestShowRoles(ctx, roleStr)
	return
}

// ManagestShowQuorums implements eth.API.
func (m *metricsWrapper) ManagestShowQuorums(ctx context.Context, actionStr string) (res map[string]interface{}, err error) {
	r, s, f, i, d := metrics.GetAPIMethodMetrics("eth_managestShowQuorums")
	defer metrics.InstrumentCaller(r, s, f, i, d, &err)()

	res, err = m.api.ManagestShowQuorums(ctx, actionStr)
	return
}

// ManagestProposalPreparing implements eth.API.
func (m *metricsWrapper) ManagestProposalPreparing(ctx context.Context, address common.Address, data hexutil.Bytes) (res map[string]interface{}, err error) {
	r, s, f, i, d := metrics.GetAPIMethodMetrics("eth_managestProposal")
	defer metrics.InstrumentCaller(r, s, f, i, d, &err)()

	res, err = m.api.ManagestProposalPreparing(ctx, address, data)
	return
}

// ManagestVotePreparing implements eth.API.
func (m *metricsWrapper) ManagestVotePreparing(ctx context.Context, address common.Address, proposalID hexutil.Uint, optionStr string) (res map[string]interface{}, err error) {
	r, s, f, i, d := metrics.GetAPIMethodMetrics("eth_managestVote")
	defer metrics.InstrumentCaller(r, s, f, i, d, &err)()

	res, err = m.api.ManagestVotePreparing(ctx, address, proposalID, optionStr)
	return
}

// send a signed runtime transaction
func (m *metricsWrapper) ManagestSendTransaction(ctx context.Context, optionStr string) (res map[string]interface{}, err error) {
	r, s, f, i, d := metrics.GetAPIMethodMetrics("eth_managesSendTransaction")
	defer metrics.InstrumentCaller(r, s, f, i, d, &err)()

	res, err = m.api.ManagestSendTransaction(ctx, optionStr)
	return
}
