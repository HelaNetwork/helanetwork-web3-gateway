...
func blockToModels(
	...
	// GB: we change the baseFee from default 10 to minPrice in a block
...
func (ib *indexBackend) StoreBlockData(ctx context.Context, oasisBlock *block.Block, txResults []*client.TransactionWithResults, blockGasLimit uint64) error { //nolint: gocyclo
	...
	// GB: define minPrice to calculate the lowest price among all the transactions and give value to BaseFee later on. 
	// in case there is no tx at all, give minPrice the defaultGasPrice, which should be updated according to requirement.
	...
	for txIndex, item := range txResults {
		...
		if len(oasisTx.AuthProofs) != 1 || oasisTx.AuthProofs[0].Module != "evm.ethereum.v0" {
			...
			if txIndex == len(txResults)-1 {
				...
				// GB: set minPrice to be defaultGasPrice only if lastTransactionPrice is 0 and not updated by previous txs, otherwise, keep the previous value.
				// as this lastTransactionPrice is the last one in this block, should be ok.
				if lastTransactionPrice.IsZero() {
					if minPrice.Cmp(quantity.NewFromUint64(math.MaxUint64)) == 0 {
						minPrice = defaultGasPrice
					}
				}else if lastTransactionPrice.Cmp(minPrice) < 0 {
					minPrice = lastTransactionPrice
				}
		...
		// GB: find the minPrice from lastTransactionPrice
		if lastTransactionPrice.Cmp(minPrice) < 0 {
			minPrice = lastTransactionPrice
		}
	...
	blk, txs, receipts, dbLogs, err := blockToModels(oasisBlock, ethTxs, logs, txsStatus, txsGasUsed, results, blockGasLimit, minPrice)
	if err != nil {
